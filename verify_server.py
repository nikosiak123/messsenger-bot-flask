...
import locale
import re
# Usunięto import defaultdict, bo nie jest już potrzebny
...
MAX_SEARCH_DAYS = 14
# PRZYWRÓCONO: Maksymalna liczba KONKRETNYCH slotów przekazywanych do AI
MAX_SLOTS_FOR_AI = 15
...
# PRZYWRÓCONO: Funkcja generująca KONKRETNE sloty
def get_free_slots(calendar_id, start_datetime, end_datetime):
    """Znajduje wolne sloty (konkretne czasy rozpoczęcia) w kalendarzu Google."""
    ...
    while current_day <= end_day:
            ...
            while potential_slot_start + appointment_duration <= busy_start:
                if potential_slot_start.minute % 10 == 0:
                     if potential_slot_start >= day_start_limit and potential_slot_start + appointment_duration <= day_end_limit:
                        ...
                        logging.debug(f"  + Wolny slot (przed): {potential_slot_start.strftime('%Y-%m-%d %H:%M')}")
        ...
        while potential_slot_start + appointment_duration <= check_end_time:
             if potential_slot_start.minute % 10 == 0:
                if potential_slot_start >= day_start_limit and potential_slot_start + appointment_duration <= day_end_limit:
                    free_slots_starts.append(potential_slot_start)
                    logging.debug(f"  + Wolny slot (po/cały): {potential_slot_start.strftime('%Y-%m-%d %H:%M')}")
             current_minute = potential_slot_start.minute
             minutes_to_add = 10 - (current_minute % 10) if current_minute % 10 != 0 else 10
             potential_slot_start += datetime.timedelta(minutes=minutes_to_add)
             potential_slot_start = potential_slot_start.replace(second=0, microsecond=0)
...
# PRZYWRÓCONO: Funkcja formatująca KONKRETNE sloty dla AI
...
# USUNIĘTO: get_valid_start_times (logika wróciła do get_free_slots)
# USUNIĘTO: is_slot_actually_free (weryfikacja będzie prostsza)
...
def book_appointment(calendar_id, start_time, end_time, summary="Rezerwacja wizyty", description="", user_name=""):
    ...
    try:
        ...
        elif error.resp.status == 403: return False, f"Brak uprawnień do zapisu w kalendarzu '{calendar_id}'. Skontaktuj się z administratorem."
        elif error.resp.status == 404: return False, f"Nie znaleziono kalendarza o ID '{calendar_id}'. Sprawdź konfigurację."
        else: return False, f"Wystąpił nieoczekiwany błąd podczas rezerwacji (kod: {error.resp.status}). Spróbuj ponownie później."
    except Exception as e:
...
# PRZYWRÓCONO: Konfiguracja dla AI wybierającego slot z listy
GENERATION_CONFIG_PROPOSAL = GenerationConfig(temperature=0.4, top_p=0.95, top_k=40, max_output_tokens=512) # 512 powinno wystarczyć na krótką propozycję
...
# PRZYWRÓCONO: Pierwotna instrukcja dla AI wybierającego z listy
...
# PRZYWRÓCONO: Logika wybierania slotu z listy
def get_gemini_slot_proposal(user_psid, history, available_slots):
    ...
    # Używamy formatowania listy slotów
    ...
    # Używamy przywróconej konfiguracji
    generated_text = _call_gemini(user_psid, prompt_content, GENERATION_CONFIG_PROPOSAL, model_purpose="Slot Proposal from List", max_retries=1)

    if not generated_text:
        return None, None

    # Walidacja - czy zawiera znacznik i czy ISO jest na liście
    iso_match = re.search(rf"\{SLOT_ISO_MARKER_PREFIX}(.*?)\{SLOT_ISO_MARKER_SUFFIX}", generated_text)
    if iso_match:
        extracted_iso = iso_match.group(1)
        # Weryfikacja, czy ISO jest na liście *przekazanej* do AI
        ...
        logging.error(f"!!! BŁĄD KRYTYCZNY AI [{user_psid}, {MODEL_ID}]: Brak znacznika {SLOT_ISO_MARKER_PREFIX}...{SLOT_ISO_MARKER_SUFFIX} w odpowiedzi AI wybierającej slot!")
                ...
                for messaging_event in entry.get("messaging", []):
                    ...
                    if messaging_event.get("message"):
                        ...
                        if text_to_send_immediately:
                            ...
                            # PRZYWRÓCONO: Logika zapisu historii po wykryciu intencji
                        ...
                        if action_to_perform == 'book':
                            try:
                                tz = _get_timezone()
                                # Prosta weryfikacja formatu ISO przed parsowaniem
                                proposed_start_dt_verify = datetime.datetime.fromisoformat(last_proposed_slot_iso).astimezone(tz)
                                # Nie ma już potrzeby weryfikacji is_slot_actually_free, bo zakładamy poprawność ISO z kontekstu
                            ...
                            try:
                                ...
                                # Krok 1: Znajdź KONKRETNE wolne sloty
                                free_slots = get_free_slots(TARGET_CALENDAR_ID, search_start, search_end)

                                if free_slots:
                                    # Krok 2: Opcjonalnie filtruj sloty wg preferencji godzinowych/popołudniowych
                                    ...
                                    if preference == 'specific_hour' and requested_hour_int is not None:
                                        ...
                                        else: logging.info(f"Brak slotów o godzinie {requested_hour_int}. Używam wszystkich {len(free_slots)}.")
                                        ...
                                        else: logging.info(f"Brak slotów popołudniowych. Używam wszystkich {len(free_slots)}.")
                                    elif preference == 'earlier':
                                         potential_matches = [s for s in free_slots if s.hour < 12]
                                         if potential_matches:
                                              filtered_slots = potential_matches
                                              logging.info(f"Wstępnie przefiltrowano sloty do przedpołudniowych. Liczba: {len(filtered_slots)}")
                                         else: logging.info(f"Brak slotów przedpołudniowych. Używam wszystkich {len(free_slots)}.")

                                    # Krok 3: Poproś AI o WYBRANIE slotu z (potencjalnie filtrowanej) listy
                                    logging.info(f"      Przekazanie {len(filtered_slots)} slotów do AI ({MODEL_ID}) w celu wyboru propozycji...")
                                    # Przekazujemy listę `filtered_slots`, AI wybierze z niej max MAX_SLOTS_FOR_AI
                                    proposal_text, proposed_iso = get_gemini_slot_proposal(sender_id, history, filtered_slots)

                                    # Krok 4: Logika awaryjna (Fallback), jeśli AI nie wybrało poprawnie
                                    if proposal_text and proposed_iso:
                                        # AI wybrało poprawny slot z listy
                                        text_to_send_as_result = proposal_text
                                        context_to_save = {'role': 'system', 'type': 'last_proposal', 'slot_iso': proposed_iso}
                                        # Logowanie już jest w get_gemini_slot_proposal
                                    else:
                                        logging.warning(f"[{sender_id}] AI ({MODEL_ID}) nie wybrało poprawnego slotu z listy. Używam logiki awaryjnej.")
                                        if filtered_slots: # Upewnij się, że mamy z czego wybrać
                                            fallback_slot = filtered_slots[0] # Wybierz pierwszy z (potencjalnie filtrowanej) listy
                                        ...
                                        else: # Ten case nie powinien się zdarzyć, jeśli free_slots nie było puste
...
if __name__ == '__main__':
    ...
    # PRZYWRÓCONO logowanie MAX_SLOTS_FOR_AI
