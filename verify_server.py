...
# --- Importy związane z Google Calendar (przeniesione z calendar_utils) ---
...
# --- Konfiguracja Kalendarza (przeniesiona z calendar_utils i app) ---
SERVICE_ACCOUNT_FILE = 'kalendarzklucz.json' # Nazwa pliku klucza konta usługi
CALENDAR_SCOPES = ['https://www.googleapis.com/auth/calendar.readonly', 'https://www.googleapis.com/auth/calendar.events'] # Odczyt i zapis
CALENDAR_TIMEZONE = 'Europe/Warsaw'
DEFAULT_APPOINTMENT_DURATION = 60 # Długość spotkania w minutach
WORK_START_HOUR = 7
WORK_END_HOUR = 22
...
# --- Ustawienia Lokalizacji (przeniesione z calendar_utils) ---
try:
...
def get_user_profile(psid):
    # <<< POPRAWKA SPRAWDZANIA TOKENA >>>
    if not PAGE_ACCESS_TOKEN or len(PAGE_ACCESS_TOKEN) < 50: # Sprawdź tylko czy jest i ma sensowną długość
         print(f"!!! [{psid}] Brak lub nieprawidłowy PAGE_ACCESS_TOKEN. Profil niepobrany."); return None
    # ----------------------------------
...
# --- Funkcje Kalendarza (przeniesione z calendar_utils.py) ---
...
def _get_timezone():
    ...
    if _tz is None:
        try: _tz = pytz.timezone(CALENDAR_TIMEZONE)
            ...
            print(f"BŁĄD: Nieznana strefa czasowa '{CALENDAR_TIMEZONE}'. Używam UTC.")
            _tz = pytz.utc
    return _tz

def get_calendar_service():
    """Tworzy lub zwraca (cachowany) obiekt usługi API Google Calendar."""
    global _calendar_service
    if _calendar_service: return _calendar_service
    creds = None
    if not os.path.exists(SERVICE_ACCOUNT_FILE): print(f"BŁĄD: Nie znaleziono pliku klucza: '{SERVICE_ACCOUNT_FILE}'"); return None
    try:
        creds = service_account.Credentials.from_service_account_file(SERVICE_ACCOUNT_FILE, scopes=CALENDAR_SCOPES)
        service = build('calendar', 'v3', credentials=creds)
        print("Pomyślnie utworzono usługę Calendar API."); _calendar_service = service; return service
    except HttpError as error: print(f"Błąd API tworzenia usługi Calendar: {error}"); return None
    except Exception as e: print(f"Nieoczekiwany błąd tworzenia usługi Calendar: {e}"); return None

def parse_event_time(event_time_data, default_tz):
    """Parsuje czas start/end z danych wydarzenia (datetime lub date)."""
    if 'dateTime' in event_time_data:
        ...
        try: dt = datetime.datetime.fromisoformat(dt_str)
            ...
            try: dt = datetime.datetime.strptime(dt_str, '%Y-%m-%dT%H:%M:%S%z')
                ...
                except ValueError: print(f"Ostrz.: Nie można sparsować dateTime: {dt_str}"); return None
        ...
        except ValueError: print(f"Ostrz.: Nie można sparsować date: {event_time_data['date']}"); return None
    return None

def get_free_slots(calendar_id, start_datetime, end_datetime, duration_minutes=DEFAULT_APPOINTMENT_DURATION):
    """Znajduje wolne sloty o zadanej długości."""
    ...
    print(f"Szukanie wolnych slotów ({duration_minutes} min) w '{calendar_id}'"); print(f"Zakres: {start_datetime:%Y-%m-%d %H:%M %Z} do {end_datetime:%Y-%m-%d %H:%M %Z}")
    try:
    ...
    while current_day <= end_day:
        ...
        for event in events:
            ...
            # <<< --- UPROSZCZONA I BEZPIECZNA LOGIKA ALL-DAY --- >>>
            if isinstance(start, datetime.date): # Wydarzenie całodniowe
                # Traktuj jako zajęty cały dzień roboczy, jeśli data startowa wydarzenia to obecny dzień
                if start == current_day:
                    busy_times.append({'start': day_start_limit, 'end': day_end_limit})
                    # print(f"   - Uwzględniono all-day: {event.get('summary','?')}")
            # <<< --- KONIEC LOGIKI ALL-DAY --- >>>
            elif isinstance(start, datetime.datetime): # Normalne wydarzenie czasowe
                end = parse_event_time(event['end'], tz)
                if isinstance(end, datetime.datetime): # Upewnij się, że koniec też jest datetime
                    if end > day_start_limit and start < day_end_limit: # Sprawdź nakładanie się z dniem roboczym
                        effective_start = max(start, day_start_limit)
                        effective_end = min(end, day_end_limit)
                        if effective_start < effective_end: # Upewnij się, że czas jest > 0
                            busy_times.append({'start': effective_start, 'end': effective_end})
                else:
                    print(f"Ostrz.: Wydarzenie czasowe '{event.get('summary','?')}' ma nieprawidłowy koniec ({type(end)})")
...
def book_appointment(calendar_id, start_time, end_time, summary="Rezerwacja wizyty", description="", user_name=""):
    ...
    try:
        ...
        except: day_names = ["Pn", "Wt", "Śr", "Cz", "Pt", "So", "Nd"]; locale_day_name = day_names[start_time.weekday()] # Fallback
...
def _send_single_message(recipient_id, message_text):
    ...
    # <<< POPRAWKA SPRAWDZANIA TOKENA >>>
    if not PAGE_ACCESS_TOKEN or len(PAGE_ACCESS_TOKEN) < 50: print(f"!!! [{recipient_id}] Brak TOKENA. Nie wysłano wiadomości."); return False
    # ----------------------------------
    try:
...
def get_gemini_response_or_action(user_psid, current_user_message):
    ...
    prompt_content_with_instruction = [
        ...
        Content(role="model", parts=[Part.from_text("Rozumiem. Będę pomagał zgodnie z wytycznymi, inicjując sprawdzanie terminów znacznikiem [ACTION: FIND_SLOTS].")])
    ] + history_to_send
