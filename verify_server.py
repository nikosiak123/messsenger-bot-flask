...
APPOINTMENT_DURATION_MINUTES = DEFAULT_APPOINTMENT_DURATION # Użyj wartości z calendar_utils (lub ustaw inną)
MAX_SLOTS_TO_SHOW = 3 # Ile wolnych terminów pokazać użytkownikowi naraz
...
def ensure_dir(directory):
    try:
        os.makedirs(directory); print(f"Utworzono katalog historii: {directory}")
    except OSError as e:
        if e.errno != errno.EEXIST: print(f"!!! Błąd podczas tworzenia katalogu {directory}: {e} !!!"); raise
                    ...
                    if (isinstance(msg_data, dict) and 'role' in msg_data and msg_data['role'] in ('user', 'model') and
                        ...
                        for part_data in msg_data['parts']:
                            if isinstance(part_data, dict) and 'text' in part_data and isinstance(part_data['text'], str):
                                ...
                                print(f"Ostrzeżenie [{user_psid}]: Niepoprawny format części w historii (indeks {i}): {part_data}.")
                                valid_parts = False; break
                        if valid_parts and text_parts: history.append(Content(role=msg_data['role'], parts=text_parts))
                    else: print(f"Ostrzeżenie [{user_psid}]: Pominięto niepoprawny format wiadomości w historii (indeks {i}): {msg_data}")
                print(f"[{user_psid}] Wczytano historię: {len(history)} wiadomości.")
                return history
            else: print(f"!!! BŁĄD [{user_psid}]: Plik historii nie zawiera listy. Zaczynam nową."); return []
    except FileNotFoundError: print(f"[{user_psid}] Nie znaleziono pliku historii. Zaczynam nową."); return []
    except (json.JSONDecodeError, KeyError, IndexError, TypeError) as e: print(f"!!! BŁĄD [{user_psid}] parsowania historii: {e}. Plik: {filepath}. Zaczynam nową."); return []
    except Exception as e: print(f"!!! Niespodziewany BŁĄD [{user_psid}] wczytywania historii: {e} !!!"); return []
...
def _send_single_message(recipient_id, message_text):
    ...
    try:
        ...
        # print(f"Odpowiedź FB API (fragment): {response_json}") # Opcjonalne
...
def send_message(recipient_id, full_message_text):
    if not full_message_text or not isinstance(full_message_text, str) or not full_message_text.strip(): print(f"[{recipient_id}] Pominięto wysyłanie pustej wiadomości."); return
    ...
    if message_len <= MESSAGE_CHAR_LIMIT: _send_single_message(recipient_id, full_message_text)
        ...
        for i, chunk in enumerate(chunks):
            ...
            if not _send_single_message(recipient_id, chunk): print(f"!!! [{recipient_id}] Anulowano wysyłanie reszty po błędzie fragm. {i+1} !!!"); break
...
def send_quick_replies(recipient_id, text, quick_replies_list):
    ...
    if not PAGE_ACCESS_TOKEN or PAGE_ACCESS_TOKEN == "TU_WSTAW_SWOJ_TOKEN_DOSTEPU_DO_STRONY":
    ...
    # Przygotuj listę przycisków dla API (limit 13 przycisków, 20 znaków tytuł, 1000 payload)
    fb_quick_replies = []
    ...
    # Przytnij tekst wiadomości, jeśli jest za długi
    ...
    try:
        ...
        r.raise_for_status()
        response_json = r.json()
        # print(f"Odpowiedź FB API (QR): {response_json}") # Opcjonalne
        ...
        # Logowanie odpowiedzi błędu (jak w _send_single_message)
        if hasattr(e, 'response') and e.response is not None:
...
# --- INSTRUKCJA SYSTEMOWA DLA GEMINI (Zaktualizowana o umawianie) ---
# TODO: Zmień [Nazwa Firmy], [Twój Numer], [Twój Email] na prawdziwe dane
...
**Ważne zasady:**
*   **Kontynuacja po przerwie:** **ZAWSZE** analizuj historię i kontynuuj rozmowę od miejsca, w którym została przerwana. **NIE ZACZYNAJ OD NOWA**, jeśli informacje są już w historii.
*   Nie używaj formy "Pan/Pani" jeśli nie masz pewności co do płci, preferuj formy bezosobowe lub "Państwo".
*   Rozdzielaj wywiad na krótsze wiadomości, zadawaj jedno główne pytanie na raz.
*   Bądź perswazyjny, ale nie nachalny. Jeśli klient odmawia, zaproponuj zastanowienie się ("Rozumiem, proszę się spokojnie zastanowić. Gdyby zmienili Państwo zdanie, jestem do dyspozycji.").
*   Jeśli nie znasz odpowiedzi (np. o dostępność konkretnego nauczyciela), powiedz: "To szczegółowa informacja, którą muszę sprawdzić. Wrócę z odpowiedzią." lub zaproponuj kontakt: tel. [Twój Numer], email: [Twój Email].
...
def get_gemini_response_or_action(user_psid, current_user_message):
    ...
    Zwraca tekst odpowiedzi LUB znacznik akcji (np. '[ACTION: FIND_SLOTS]') LUB None w razie błędu.
    """
    if not gemini_model:
        print(f"!!! KRYTYCZNY BŁĄD [{user_psid}]: Model Gemini niezaładowany!")
        history = load_history(user_psid) # Załaduj historię, by ją zapisać przed błędem
        user_content = Content(role="user", parts=[Part.from_text(current_user_message)])
        save_history(user_psid, history + [user_content])
        return "Przepraszam, wystąpił wewnętrzny problem techniczny (AI niedostępne)."

    # --- Ładowanie i przygotowanie historii (jak w dostarczonym kodzie) ---
    history = load_history(user_psid)
    user_content = Content(role="user", parts=[Part.from_text(current_user_message)])
    full_conversation_for_save = history + [user_content] # Pełna historia do zapisu
    ...
    if len(full_conversation_for_save) > max_messages_to_send:
         print(f"[{user_psid}] Historia przycięta DO WYSLANIA do Gemini: {len(history_to_send)} wiadomości.")

    # --- Przygotowanie promptu z instrukcją (jak w dostarczonym kodzie) ---
    ...
    # --- Logowanie wysyłanej zawartości (jak w dostarczonym kodzie) ---
    ...
    try:
        # --- Konfiguracja i generowanie (jak w dostarczonym kodzie) ---
        generation_config = GenerationConfig(temperature=0.7, top_p=0.95, top_k=40) # max_output_tokens domyślny dla flash
        safety_settings = { # Zdefiniowane, by być pewnym
        ...
        generated_text = ""
        # --- Przetwarzanie odpowiedzi (jak w dostarczonym kodzie, ale z dodaną obsługą akcji) ---
        if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
            ...
            if generated_text == "[ACTION: FIND_SLOTS]":
                ...
                # Zapisz historię *bez* znacznika jako odpowiedzi modelu
                save_history(user_psid, full_conversation_for_save)
                return "[ACTION: FIND_SLOTS]" # Zwróć znacznik
            # <<< --- KONIEC NOWEJ LOGIKI --- >>>

            # Jeśli to normalna odpowiedź, kontynuuj jak poprzednio
            ...
            # Przycinanie historii DO ZAPISU (jak w dostarczonym kodzie)
            ...
            return generated_text # Zwróć normalny tekst

        else:
            # --- Obsługa błędów / blokad Gemini (jak w dostarczonym kodzie) ---
            finish_reason = "UNKNOWN"; safety_ratings_str = "Brak"
            # (Logika pobierania finish_reason i safety_ratings bez zmian)
            if response.candidates:
                 candidate = response.candidates[0]; finish_reason_obj = candidate.finish_reason
                 finish_reason = finish_reason_obj.name if hasattr(finish_reason_obj, 'name') else str(finish_reason_obj)
                 if candidate.safety_ratings: safety_ratings_str = ", ".join([f"{r.category.name}: {r.probability.name}" for r in candidate.safety_ratings])
            print(f"!!! [{user_psid}] Odpowiedź Gemini pusta/zablokowana. Powód: {finish_reason}, Oceny: {safety_ratings_str} !!!")
            save_history(user_psid, full_conversation_for_save) # Zapisz historię do wiad. użytkownika
            # (Logika zwracania komunikatów o błędach bez zmian)
            ...
            else: return "Hmm, nie udało mi się wygenerować odpowiedzi. Spróbuj inaczej sformułować pytanie."

    except Exception as e:
        # --- Obsługa wyjątków (jak w dostarczonym kodzie) ---
        import traceback
        print(f"!!! KRYTYCZNY BŁĄD Gemini ({MODEL_ID}) dla PSID {user_psid}: {e} !!!"); traceback.print_exc()
        save_history(user_psid, full_conversation_for_save) # Zapisz historię do wiad. użytkownika
        # (Logika zwracania komunikatów o błędach bez zmian)
        error_str = str(e).lower()
        if "permission denied" in error_str: return "Błąd: Brak uprawnień do użycia AI."
        elif "model" in error_str and "not found" in error_str: return f"Błąd: Model AI ('{MODEL_ID}') niedostępny."
        elif "deadline exceeded" in error_str: return "Błąd: AI nie odpowiedziało na czas."
        elif "quota" in error_str: return "Błąd: Osiągnięto limit zapytań AI."
        elif "content" in error_str and "invalid" in error_str: return "Błąd: Wewnętrzny błąd formatowania AI."
...
def webhook_handle():
    ...
    raw_data = request.data.decode('utf-8'); data = None # Przenieś inicjalizację data
    try:
                ...
                for messaging_event in entry.get("messaging", []):
                    ...
                    if messaging_event.get("message"):
                        ...
                        if "quick_reply" in message_data:
                            ...
                            user_input_text = message_data.get("text", quick_reply_payload) # Tekst przycisku lub payload
                        ...
                        # --- LOGIKA REZERWACJI (jeśli kliknięto przycisk BOOK_SLOT) ---
                        if quick_reply_payload and quick_reply_payload.startswith(QUICK_REPLY_BOOK_PREFIX):
                            ...
                            try:
                                tz = _get_timezone() # Pobierz strefę czasową
                                ...
                                # Opcjonalne: Pobierz imię użytkownika
                                # user_profile = get_user_profile(sender_id)
                                # user_name = user_profile.get('first_name', '') if user_profile else ''
                                user_name = "" # Na razie bez pobierania profilu

                                if ENABLE_TYPING_DELAY: time.sleep(MIN_TYPING_DELAY_SECONDS) # Krótkie opóźnienie "myślenia"
                                ...
                                send_message(sender_id, message_to_user) # Wyślij wynik

                            except ValueError as ve: print(f"!!! BŁĄD [{sender_id}] parsowania ISO QR payload: {slot_iso_string}. Błąd: {ve} !!!"); send_message(sender_id, "Błąd przetwarzania wybranego terminu.")
                            except Exception as book_err: print(f"!!! KRYTYCZNY BŁĄD [{sender_id}] rezerwacji: {book_err} !!!"); import traceback; traceback.print_exc(); send_message(sender_id, "Niespodziewany błąd systemu rezerwacji.")

                        # --- Jeśli to nie była rezerwacja -> Przetwarzanie przez Gemini ---
                        elif user_input_text:
                             print(f"      Przekazywanie tekstu do Gemini...")
                             gemini_output = get_gemini_response_or_action(sender_id, user_input_text)

                             # --- OBSŁUGA AKCJI FIND_SLOTS ---
                             if gemini_output == "[ACTION: FIND_SLOTS]":
                                 print(f"      Wykonywanie akcji: FIND_SLOTS")
                                 tz = _get_timezone()
                                 now = datetime.datetime.now(tz)
                                 search_start = now # Szukaj od teraz
                                 # Szukaj np. przez 7 dni, kończąc o 23:59 ostatniego dnia
                                 search_end_date = (now + datetime.timedelta(days=7)).date()
                                 ...
                                 if free_slots:
                                     replies = []
                                     print(f"      Znaleziono {len(free_slots)} slotów. Przygotowywanie QR...")
                                     for slot_start in free_slots[:MAX_SLOTS_TO_SHOW]:
                                         try: # Użyj locale do formatowania dnia tygodnia
                                             slot_text = slot_start.strftime("%A, %d.%m %H:%M") # np. Poniedziałek, 01.07 14:00
                                         except: # Fallback
                                             day_names = ["Pon", "Wt", "Śr", "Czw", "Pt", "Sob", "Niedz"]
                                             slot_text = f"{day_names[slot_start.weekday()]}, {slot_start.strftime('%d.%m %H:%M')}"
                                         replies.append({
                                             "title": slot_text,
                                             "payload": f"{QUICK_REPLY_BOOK_PREFIX}{slot_start.isoformat()}"
                                         })
                                     message_text = "Jasne, oto kilka najbliższych dostępnych terminów. Wybierz jeden:"
                                     send_quick_replies(sender_id, message_text, replies)
                                 else:
                                     print(f"      Nie znaleziono wolnych slotów do {search_end_date}.")
                                     send_message(sender_id, "Niestety, w najbliższym czasie (kolejne 7 dni) brak wolnych terminów. Sprawdź ponownie później lub skontaktuj się z nami.")
                             ...
                             # --- Jeśli to normalna odpowiedź Gemini ---
                                 ...
                                 if ENABLE_TYPING_DELAY: # Symulacja pisania
                                     response_len = len(gemini_output)
                                 ...
                                 send_message(sender_id, gemini_output) # Wyślij odpowiedź
                             else: # Błąd krytyczny w Gemini
                                  print(f"!!! [{sender_id}] get_gemini_response_or_action zwróciło błąd. Wysyłanie generycznego komunikatu.")
                                  send_message(sender_id, "Przepraszam, wystąpił błąd podczas przetwarzania wiadomości.")

                        # --- Obsługa załączników ---
                        elif "attachments" in message_data:
                             attachment_type = message_data['attachments'][0].get('type', 'nieznany')
                             print(f"      Odebrano załącznik: {attachment_type}.")
                             send_message(sender_id, "Przepraszam, nie obsługuję załączników.")
                        # --- Inne nieobsługiwane wiadomości ---
                        else:
                            print(f"      Odebrano nieznany typ wiadomości: {message_data}")
                            send_message(sender_id, "Otrzymałem wiadomość, ale nie jestem pewien, co z nią zrobić.")


                    # --- Obsługa innych zdarzeń (Postback, Read, Delivery) ---
                    # (Logika bez zmian w stosunku do dostarczonego kodu)
                    elif messaging_event.get("postback"):
                         postback_data = messaging_event["postback"]; payload = postback_data.get("payload")
                         title = postback_data.get("title", payload)
                         print(f"    Typ zdarzenia: postback. Tytuł: '{title}', Payload: '{payload}'")
                         # TODO: Można dodać logikę obsługi konkretnych payloadów postback
                         send_message(sender_id, f"Otrzymałem akcję: '{title}'.")
        ...
        print(f"!!! KRYTYCZNY BŁĄD: Nie można zdekodować JSON: {json_err} !!!"); print(f"   Surowe dane (max 500): {raw_data[:500]}"); return Response("Invalid JSON", status=400)
    except Exception as e:
        import traceback
        print(f"!!! KRYTYCZNY BŁĄD przetwarzania POST: {e} !!!"); traceback.print_exc()
    return Response("EVENT_RECEIVED", status=200) # Zawsze 200 OK dla FB


# --- Uruchomienie Serwera ---
# (Logika startowa bez zmian, ale dodano walidację tokena FB)
if __name__ == '__main__':
    ...
    # Walidacja tokenów przy starcie
    ...
    if not PAGE_ACCESS_TOKEN or len(PAGE_ACCESS_TOKEN) < 50: # Prosta walidacja długości
         print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
         print("!!! KRYTYCZNE OSTRZEŻENIE: FB_PAGE_ACCESS_TOKEN PUSTY/NIEPOPRAWNY?!")
         print("!!! Bot NIE BĘDZIE DZIAŁAŁ poprawnie!                       !!!")
         print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    ...
    if debug_mode: app.run(host='0.0.0.0', port=port, debug=True)
        ...
        # W produkcji użyj serwera WSGI, np. waitress
        try:
            from waitress import serve
            print("Uruchamianie serwera produkcyjnego Waitress...")
            serve(app, host='0.0.0.0', port=port)
        except ImportError:
            print("Waitress nie jest zainstalowany. Uruchamianie wbudowanego serwera Flask (niezalecane na produkcji).")
