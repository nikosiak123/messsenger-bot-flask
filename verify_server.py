...
# verify_server.py (Architektura stanów: Ogólny <-> Planista <-> Feedback)
...
# ZMIANA: Konfiguracja dla AI Proponującego (jeszcze niższa temp?)
...
# =====================================================================
# === FUNKCJE POMOCNICZE (Bez zmian od poprzedniej wersji) ============
# =====================================================================
# (Zakładamy, że funkcje pomocnicze: ensure_dir, get_user_profile, load_history,
# save_history, _get_timezone, get_calendar_service, parse_event_time,
# get_free_time_ranges, is_slot_actually_free, book_appointment,
# format_ranges_for_ai, format_slot_for_user, _send_typing_on,
# _send_single_message, send_message, _simulate_typing, _call_gemini
# są zdefiniowane tak jak w poprzednich odpowiedziach)
...
def get_user_profile(psid):
    ...
    try:
        ...
        return profile_data
    except requests.exceptions.Timeout: logging.error(f"BŁĄD TIMEOUT podczas pobierania profilu {psid}"); return None
         ...
         return None
    except requests.exceptions.RequestException as req_err: logging.error(f"BŁĄD RequestException podczas pobierania profilu {psid}: {req_err}"); return None
    except Exception as e: logging.error(f"Niespodziewany BŁĄD podczas pobierania profilu {psid}: {e}", exc_info=True); return None
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            if isinstance(history_data, list):
                ...
                for i, msg_data in enumerate(history_data):
                    ...
                    # ZMIANA: Odczyt różnych typów kontekstu/stanu
                ...
                return history, context
            else: logging.error(f"BŁĄD [{user_psid}]: Plik historii nie zawiera listy."); return [], {}
...
def save_history(user_psid, history, context_to_save=None):
    ...
    try:
        ...
        for msg in history_to_process:
             ...
             else: logging.warning(f"Ostrz. [{user_psid}]: Pomijanie nieprawidłowego obiektu historii: {msg}")

        # ZMIANA: Zapisz kontekst tylko jeśli nie jest pusty (co oznacza stan general)
        ...
        if os.path.exists(temp_filepath):
            try: os.remove(temp_filepath); logging.info(f"    Usunięto plik tymczasowy.")
            except OSError as remove_e: logging.error(f"    Nie można usunąć pliku tymczasowego {temp_filepath}: {remove_e}")
...
def get_calendar_service():
    ...
    if not os.path.exists(SERVICE_ACCOUNT_FILE): logging.error(f"KRYTYCZNY BŁĄD: Brak pliku klucza: '{SERVICE_ACCOUNT_FILE}'"); return None
    try:
        creds = service_account.Credentials.from_service_account_file(SERVICE_ACCOUNT_FILE, scopes=CALENDAR_SCOPES)
        service = build('calendar', 'v3', credentials=creds, cache_discovery=False)
        logging.info("Utworzono połączenie z Google Calendar API."); _calendar_service = service; return service
    except HttpError as error: logging.error(f"Błąd HTTP tworzenia usługi Calendar: {error}", exc_info=True); return None
    ...
    if 'dateTime' in event_time_data:
        ...
        try: dt = datetime.datetime.fromisoformat(dt_str)
            ...
            if dt_str.endswith('Z'):
                try: dt = datetime.datetime.strptime(dt_str, '%Y-%m-%dT%H:%M:%S.%fZ'); dt = pytz.utc.localize(dt)
                except ValueError:
                     try: dt = datetime.datetime.strptime(dt_str, '%Y-%m-%dT%H:%M:%SZ'); dt = pytz.utc.localize(dt)
                     except ValueError: logging.warning(f"Ostrz.: Nie sparsowano dateTime (z Z): {dt_str}"); return None
            else:
                try:
                    if ':' in dt_str[-6:]: dt_str = dt_str[:-3] + dt_str[-2:]
...
def get_free_time_ranges(calendar_id, start_datetime, end_datetime):
    ...
    if start_datetime >= end_datetime: logging.info("Zakres wyszukiwania wolnych terminów nieprawidłowy."); return []
    ...
    try:
        ...
        for busy_slot in busy_times_raw:
            try:
                busy_start = datetime.datetime.fromisoformat(busy_slot['start']).astimezone(tz); busy_end = datetime.datetime.fromisoformat(busy_slot['end']).astimezone(tz)
            ...
            except ValueError as e: logging.warning(f"Ostrz.: Nie sparsowano zajętego czasu: {busy_slot}, błąd: {e}")
    except HttpError as error: logging.error(f'Błąd API Google Calendar (Freebusy): {error}', exc_info=True); return []
    except Exception as e: logging.error(f"Nieoczekiwany błąd Freebusy: {e}", exc_info=True); return []
    ...
    final_free_slots = []; min_duration_delta = datetime.timedelta(minutes=APPOINTMENT_DURATION_MINUTES)
    for free_range in free_ranges:
        range_start = free_range['start']; range_end = free_range['end']; current_day_start = range_start
        while current_day_start < range_end:
            day_date = current_day_start.date(); work_day_start = tz.localize(datetime.datetime.combine(day_date, datetime.time(WORK_START_HOUR, 0))); work_day_end = tz.localize(datetime.datetime.combine(day_date, datetime.time(WORK_END_HOUR, 0)))
            intersect_start = max(current_day_start, work_day_start); intersect_end = min(range_end, work_day_end)
            if intersect_start < intersect_end and (intersect_end - intersect_start) >= min_duration_delta:
                if intersect_start.minute % 10 != 0 or intersect_start.second > 0 or intersect_start.microsecond > 0:
                    minutes_to_add = 10 - (intersect_start.minute % 10); rounded_start = intersect_start + datetime.timedelta(minutes=minutes_to_add); rounded_start = rounded_start.replace(second=0, microsecond=0)
                else: rounded_start = intersect_start
                if rounded_start < intersect_end and (intersect_end - rounded_start) >= min_duration_delta: final_free_slots.append({'start': rounded_start, 'end': intersect_end})
            next_day_date = day_date + datetime.timedelta(days=1); current_day_start = tz.localize(datetime.datetime.combine(next_day_date, datetime.time(0, 0))); current_day_start = max(current_day_start, range_start)
    logging.info(f"Znaleziono {len(final_free_slots)} wolnych zakresów czasowych."); return final_free_slots

def is_slot_actually_free(start_time, calendar_id):
    service = get_calendar_service(); tz = _get_timezone()
    if not service: logging.error("Błąd: Usługa kalendarza niedostępna."); return False
    ...
    try:
        ...
        if not busy_times: logging.info(f"Weryfikacja: Slot {start_time:%Y-%m-%d %H:%M} jest wolny."); return True
            ...
            for busy in busy_times:
                busy_start = datetime.datetime.fromisoformat(busy['start']).astimezone(tz); busy_end = datetime.datetime.fromisoformat(busy['end']).astimezone(tz)
                if max(start_time, busy_start) < min(end_time, busy_end): logging.warning(f"Weryfikacja: Slot {start_time:%Y-%m-%d %H:%M} jest ZAJĘTY."); return False
            logging.info(f"Weryfikacja: Slot {start_time:%Y-%m-%d %H:%M} jest wolny (brak kolizji)."); return True
    except HttpError as error: logging.error(f'Błąd API Google Calendar (Freebusy) weryfikacji: {error}', exc_info=True); return False
    except Exception as e: logging.error(f"Nieoczekiwany błąd weryfikacji Freebusy: {e}", exc_info=True); return False
...
def book_appointment(calendar_id, start_time, end_time, summary="Rezerwacja FB", description="", user_name=""):
    ...
    try:
        ...
        created_event = service.events().insert(calendarId=calendar_id, body=event).execute(); event_id = created_event.get('id')
        ...
        try: error_content = json.loads(error.content.decode('utf-8')); error_message = error_content.get('error', {}).get('message', ''); error_details += f" - {error_message}" if error_message else ""
        ...
        elif error.resp.status == 403: return False, "Problem z uprawnieniami do zapisu w kalendarzu."
        elif error.resp.status == 404: return False, f"Nie znaleziono kalendarza ('{calendar_id}')."
        elif error.resp.status == 400: return False, f"Błąd danych rezerwacji. ({error_details})"
        else: return False, f"Nieoczekiwany błąd ({error.resp.status}) rezerwacji."
    except Exception as e: logging.error(f"Nieoczekiwany błąd Python rezerwacji: {e}", exc_info=True); return False, "Wewnętrzny błąd systemu rezerwacji."
...
def format_ranges_for_ai(ranges):
    ...
    for r in ranges: range_date = r['start'].date(); start_time = r['start'].astimezone(tz); end_time = r['end'].astimezone(tz); ranges_by_date[range_date].append({'start_time': start_time.strftime('%H:%M'), 'end_time': end_time.strftime('%H:%M')})
    formatted_lines = [f"Dostępne ZAKRESY czasowe (wizyta trwa {APPOINTMENT_DURATION_MINUTES} minut). Wybierz JEDEN zakres i wygeneruj DOKŁADNY termin startu (preferuj pełne godziny), dołączając go w znaczniku [SLOT_ISO:...].", "--- Dostępne Zakresy ---"]
    ...
    for d in sorted(ranges_by_date.keys()):
        ...
        if time_ranges_str: formatted_lines.append(f"- {day_name}, {date_str}: {time_ranges_str}"); dates_added += 1
            if dates_added >= max_dates_to_show: formatted_lines.append("- ... (i potencjalnie więcej)"); break
...
def _send_single_message(recipient_id, message_text):
    ...
    try:
        ...
        if response_json.get('error'):
            fb_error = response_json['error']; logging.error(f"!!! BŁĄD FB API wysyłania: {fb_error} !!!")
            if fb_error.get('code') == 190: logging.error("!!! POTWIERDZONO BŁĄD TOKENA (code 190) !!!")
            return False
        logging.debug(f"[{recipient_id}] Fragment wysłany."); return True
    except requests.exceptions.Timeout: logging.error(f"!!! TIMEOUT wysyłania do {recipient_id} !!!"); return False
        ...
        return False
    except requests.exceptions.RequestException as e: logging.error(f"!!! BŁĄD sieciowy wysyłania do {recipient_id}: {e} !!!"); return False
...
def send_message(recipient_id, full_message_text):
    ...
    if ENABLE_TYPING_DELAY:
        ...
        logging.debug(f"[{recipient_id}] Czas pisania: {est_dur:.2f}s"); _send_typing_on(recipient_id); time.sleep(est_dur)
    chunks = []
    ...
    for i, chunk in enumerate(chunks):
        ...
        if num_chunks > 1 and i < num_chunks - 1:
            ...
            if ENABLE_TYPING_DELAY:
                ...
                _send_typing_on(recipient_id); time.sleep(est_dur + MESSAGE_DELAY_SECONDS * 0.5); time.sleep(MESSAGE_DELAY_SECONDS * 0.5)
...
def _call_gemini(user_psid, prompt_history, generation_config, task_name, max_retries=3):
    ...
    if not isinstance(prompt_history, list) or not all(isinstance(item, Content) for item in prompt_history): logging.error(f"!!! [{user_psid}] Nieprawidłowy prompt dla Gemini ({task_name})."); return None
        ...
        try:
            ...
            if response and response.candidates:
                ...
                if response.candidates[0].content and response.candidates[0].content.parts:
                    ...
                    logging.info(f"[{user_psid}] Gemini ({task_name}) zwróciło odp. (dł: {len(generated_text)})."); return generated_text.strip()
...
""".format(
    ...
    # Usunięto pref_weekday/pref_weekend - AI ma wywnioskować z historii lub wybrać cokolwiek
)
...
def get_gemini_slot_proposal(user_psid, history_for_proposal_ai, available_ranges):
    ...
    # ZMIANA: Użycie uproszczonej instrukcji
    ...
    initial_prompt = [
        ...
        Content(role="model", parts=[Part.from_text("OK. Wybieram jeden termin i zwracam propozycję z [SLOT_ISO:...].")]) # Krótsze potwierdzenie
    ]
        ...
        if data and data.get("object") == "page":
            for entry in data.get("entry", []):
                for event in entry.get("messaging", []):
                    ...
                    current_state = context.get('type', STATE_GENERAL) # Odczytaj bieżący stan
                    ...
                    ctx_save_payload = context # Domyślnie zachowaj dane kontekstu
                    ...
                    if message_data := event.get("message"):
                        ...
                        if user_input_text:
                            ...
                            if current_state == STATE_WAITING_FOR_FEEDBACK:
                                ...
                                try:
                                    ...
                                    if decision == "[ACCEPT]":
                                        ...
                                        next_state = STATE_SCHEDULING_ACTIVE # Przejdź do stanu aktywnego szukania
                                        ctx_save_payload = {} # Reset starego ISO
                                    elif decision == "[CLARIFY]":
                                        # ZMIANA: Przekierowanie do AI Ogólnego
                                        ...
                                        next_state = STATE_GENERAL # Wracamy do stanu ogólnego
                                        ctx_save_payload = {} # Reset kontekstu
                                    else:
                                        action = 'send_error'
                            ...
                            else: # Stan STATE_GENERAL lub STATE_SCHEDULING_ACTIVE (traktujemy tak samo - AI ogólne decyduje)
                                ...
                                if response:
                                    if INTENT_SCHEDULE_MARKER in response:
                                        ...
                                        next_state = STATE_SCHEDULING_ACTIVE # Przejdź do stanu szukania
                                        ctx_save_payload = {} # Reset danych kontekstu
                                    else:
                                        action = 'send_gemini_response'
                                        msg_result = response
                                        next_state = STATE_GENERAL # Pozostań/wróć do stanu ogólnego
                                        ctx_save_payload = {}
                                else:
                                    action = 'send_error'
                        ...
                        # Puste wiadomości / Załączniki
                             ...
                             action = 'send_info'
                             # Stan i kontekst pozostają bez zmian
                             next_state = current_state
                             ctx_save_payload = context
                        else:
                            ...
                            if current_state == STATE_WAITING_FOR_FEEDBACK:
                                ...
                                action = None # Ignoruj w stanie general/scheduling_active
                                next_state = current_state
                                ctx_save_payload = context
                    ...
                    # === Obsługa Postback ===
                        ...
                        if payload == "ACCEPT_SLOT":
                            if current_state == STATE_WAITING_FOR_FEEDBACK and last_iso_from_context:
                                ...
                                action = 'book'; msg_result = None
                                next_state = STATE_GENERAL; ctx_save_payload = {}
                            else:
                                logging.warning("      Postback 'ACCEPT_SLOT' w złym stanie lub bez ISO.")
                                action = 'send_info'; msg_result = "Propozycja wygasła."; next_state = STATE_GENERAL; ctx_save_payload = {}
                        elif payload == "REJECT_SLOT":
                            if current_state == STATE_WAITING_FOR_FEEDBACK and last_iso_from_context:
                                logging.info("      Postback: Odrzucenie -> Akcja: Szukanie, Stan: SchedulingActive")
                                action = 'find_and_propose'; msg_result = "OK, szukam innego terminu..."
                                next_state = STATE_SCHEDULING_ACTIVE; ctx_save_payload = {}
                            else:
                                logging.warning("      Postback 'REJECT_SLOT' w złym stanie lub bez ISO.")
                                action = 'send_info'; msg_result = "Brak propozycji do odrzucenia."; next_state = STATE_GENERAL; ctx_save_payload = {}
                        else: # Inne postbacki traktuj jak ogólne zapytanie
                    ...
                    if action == 'book':
                        if last_iso_from_context:
                            # (Logika book_appointment bez zmian)
                            try:
                                ...
                                ok, booking_msg = book_appointment(TARGET_CALENDAR_ID, start, end, f"FB: {name}", desc, name); msg_result = booking_msg
                                if not ok: next_state = STATE_GENERAL; ctx_save_payload = {} # Reset jeśli błąd
                        ...
                        try:
                            ...
                            if free_ranges:
                                ...
                                proposal_text, proposed_iso = get_gemini_slot_proposal(sender_id, history_for_proposal_ai, free_ranges) # Używa uproszczonej instrukcji
                                if proposal_text and proposed_iso:
                                    ...
                                    # Ustaw stan oczekiwania na feedback i zapisz ISO
                                    next_state = STATE_WAITING_FOR_FEEDBACK
                                    ctx_save_payload = {'slot_iso': proposed_iso}
                                else: # AI nie zwróciło ISO
                                     logging.warning(f"      AI (SimpleInstr) nie zwróciło ISO.")
                            ...
                            next_state = STATE_GENERAL; ctx_save_payload = {}
                    # ZMIANA: Nowa akcja obsługi przekierowania z CLARIFY lub nieznanego postbacka
                    elif action == 'send_general_ai_response':
                         logging.info(f"      -> Akcja: Przekazanie do AI General...")
                         # User content powinno zawierać wiadomość, która spowodowała CLARIFY lub nieznany postback
                         if user_content and user_content.parts:
                             ...
                             if response:
                                 if INTENT_SCHEDULE_MARKER in response:
                                     # Jeśli AI General po odpowiedzi na pytanie od razu wykryje intencję, przechodzimy do szukania
                                     logging.info(f"      AI General odpowiedziało i wykryło intencję [{INTENT_SCHEDULE_MARKER}].")
                                     action = 'find_and_propose' # Zmień akcję na szukanie
                                     initial_resp_text = response.split(INTENT_SCHEDULE_MARKER, 1)[0].strip()
                                     msg_result = initial_resp_text if initial_resp_text else "Sprawdzę terminy."
                                     next_state = STATE_SCHEDULING_ACTIVE
                                     ctx_save_payload = {}
                                     # Ponieważ zmieniliśmy akcję, musimy ponownie wykonać blok 'find_and_propose'
                                     # To jest trochę niezgrabne, może wymagać refaktoryzacji w przyszłości
                                     # Na razie skopiujemy logikę find_and_propose (bez try-except)
                                     tz = _get_timezone(); now = datetime.datetime.now(tz); search_start = now; search_end_date = (search_start + datetime.timedelta(days=MAX_SEARCH_DAYS)).date(); search_end = tz.localize(datetime.datetime.combine(search_end_date, datetime.time(WORK_END_HOUR, 0)))
                                     logging.info(f"      -> (Po CLARIFY) Szukanie od {search_start:%Y-%m-%d %H:%M} do {search_end:%Y-%m-%d %H:%M}")
                                     _simulate_typing(sender_id, MAX_TYPING_DELAY_SECONDS * 0.8)
                                     free_ranges = get_free_time_ranges(TARGET_CALENDAR_ID, search_start, search_end)
                                     if free_ranges:
                                         logging.info(f"      (Po CLARIFY) Znaleziono {len(free_ranges)} zakresów. AI (Proposal SimpleInstr)...")
                                         history_for_proposal_ai = history_for_gemini + ([user_content] if user_content else []) # Historia zawiera też CLARIFY msg
                                         proposal_text, proposed_iso = get_gemini_slot_proposal(sender_id, history_for_proposal_ai, free_ranges)
                                         if proposal_text and proposed_iso: final_proposal_msg = (msg_result + "\n\n" + proposal_text) if msg_result else proposal_text; msg_result = final_proposal_msg; next_state = STATE_WAITING_FOR_FEEDBACK; ctx_save_payload = {'slot_iso': proposed_iso}
                                         elif proposal_text and not proposed_iso: logging.warning(f"      AI (SimpleInstr, po CLARIFY) dało tekst '{proposal_text[:50]}...' bez ISO."); final_proposal_msg = (msg_result + "\n\n" + proposal_text) if msg_result else proposal_text; msg_result = final_proposal_msg; next_state = STATE_GENERAL; ctx_save_payload = {}
                                         else: fail_msg = proposal_text if proposal_text else "Problem ze znalezieniem terminu."; msg_result = (msg_result + "\n\n" + fail_msg) if msg_result else fail_msg; next_state = STATE_GENERAL; ctx_save_payload = {}
                                     else: logging.warning(f"      (Po CLARIFY) Brak wolnych zakresów."); no_slots_msg = f"Niestety, brak wolnych terminów w ciągu {MAX_SEARCH_DAYS} dni."; msg_result = (msg_result + "\n\n" + no_slots_msg) if msg_result else no_slots_msg; next_state = STATE_GENERAL; ctx_save_payload = {}

                                 else: # Zwykła odpowiedź AI General
                                     action = 'send_gemini_response' # Ustaw akcję z powrotem
                                     msg_result = response
                                     next_state = STATE_GENERAL
                                     ctx_save_payload = {}
                             ...
                             action = 'send_error'; msg_result = "Wewnętrzny błąd."; next_state = STATE_GENERAL; ctx_save_payload = {}

                    elif action in ['send_gemini_response', 'send_clarification', 'send_error', 'send_info']:
                        logging.debug(f"      Akcja: {action}. Wiadomość gotowa.")
                        # msg_result już ustawiony
                        # next_state i ctx_save_payload powinny być już ustawione
                        pass
                    ...
                    # Sprawdź, czy stan lub konwersacja się zmieniły
                    should_save = bool(user_content) or bool(model_resp_content) or (context != final_context_to_save)
...
if __name__ == '__main__':
    ...
    if not cal_service and os.path.exists(SERVICE_ACCOUNT_FILE): print("!!! OSTRZ.: Usługa Google Calendar NIE zainicjowana. Sprawdź uprawnienia/API. !!!")
